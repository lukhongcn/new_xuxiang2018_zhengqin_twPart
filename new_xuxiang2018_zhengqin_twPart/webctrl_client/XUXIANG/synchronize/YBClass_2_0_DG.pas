...
type
  TTransferation_version_2_0_DG = class(TObject)
    private
...
{ TTransferation_version_2_0_DG }

procedure TTransferation_version_2_0_DG.setMsSqlConnectionString(
  AMsSqlConnectionString : String);
begin
  MsSqlConnectionString := AMsSqlConnectionString;
end;

procedure TTransferation_version_2_0_DG.setMySqlConnectionString(
  AMySqlConnectionString: String);
begin
  MySqlConnectionString := AMySqlConnectionString;
end;

procedure TTransferation_version_2_0_DG.updateBreakTimeList;
var
...
procedure TTransferation_version_2_0_DG.updateEmployeeList;
var
  min, max, i : integer;
...
begin
  ...
  begin
    ...
    if ADOQuerySelect.Active then ADOQuerySelect.Active := false;
    ADOQuerySelect.ConnectionString := MsSqlConnectionString;
    ADOQuerySelect.SQL.Text := 'SELECT emp_id,emp_name,card_no FROM Emp where dept_code=''09'' ';
    ADOQuerySelect.Open ;
    ...
    try
      ...
      begin
        ...
        if max > 0 then
          for i := min to max-1 do
          begin
            CEmployee.setEmplID(FixData(FieldByName('emp_id').AsString));
            CEmployee.setName(FieldByName('emp_name').AsString);
            CEmployee.setCardNO(FieldByName('card_no').AsString);
            ...
            DelEmployeeList := DelEmployeeList +''''+ FixData(FieldByName('emp_id').AsString) + ''',';
            GUser.AddProgress(1);
            Application.ProcessMessages ;
            Next;
          end;
...
procedure TTransferation_version_2_0_DG.updateShiftList;
var
  min, max, i : integer;
  CShift : TShifts;
  DelShiftList : String;
...
procedure TTransferation_version_2_0_DG.updateWorkTimeList(ACurrentDate : TDateTime);
var
  i : integer;
