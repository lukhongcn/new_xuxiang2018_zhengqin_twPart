...
namespace ModuleWorkFlow
{
    ...
    public class SubGroupGetStation : System.Web.UI.Page
    {
        ...
        private void BindProcess()
        {
            ...
            drp_processlist.DataSource = process.GetNoMachineProcessInfo().Cast<Model.ProcessInfo>().ToList().FindAll(m=>m.PriceType.Equals("SUB") );
        ...
        private string ValidateContinuousSelectedProcesses(IEnumerable<PartPartProcessInfo> selectedProcessInfos)
        {
            foreach (var processGroup in selectedProcessInfos
                .GroupBy(m => new { m.ModuleId, m.PartNo_Id }))
            {
                List<PartPartProcessInfo> orderedProcessInfos = processGroup
                    .OrderBy(m => m.ProcessOrder)
                    .ToList();
                for (int i = 1; i < orderedProcessInfos.Count; i++)
                {
                    if (orderedProcessInfos[i].ProcessOrder != orderedProcessInfos[i - 1].ProcessOrder + 1)
                    {
                        return $"廠批[{orderedProcessInfos[i].ModuleId}]圖號[{orderedProcessInfos[i].PartNo_Id}]後續工站必須連續選中，不能跳工站選中";
                    }
                }
            }
        ...
        private List<PartPartProcessInfo> GetCurrentSelectedProcesses(IEnumerable<PartPartProcessInfo> selectedProcessInfos)
        {
            return selectedProcessInfos
                .GroupBy(m => new { m.ModuleId, m.PartNo_Id })
                .Select(m => m.OrderBy(n => n.ProcessOrder).First())
                .ToList();
        }

        private List<PartPartProcessInfo> GetFollowingSelectedProcesses(IEnumerable<PartPartProcessInfo> selectedProcessInfos)
        {
            return selectedProcessInfos
                .GroupBy(m => new { m.ModuleId, m.PartNo_Id })
                .SelectMany(m => m.OrderBy(n => n.ProcessOrder).Skip(1))
                .ToList();
        }

        private List<string> BuildFollowingActionSequence(string finalActionId)
        {
            List<string> actionIds = new List<string> { "JIEDAN", "KAISHI" };
            if (finalActionId.Equals("XIAODAN"))
            {
                actionIds.Add("JIESHU");
            }

            actionIds.Add(finalActionId);
            return actionIds;
        }

        private string GetActionNameById(string actionId)
        {
            for (int i = 0; i < PTSetting.ACTIONBARCODE.Length; i++)
            {
                if (PTSetting.ACTIONBARCODE[i].Equals(actionId))
                {
                    return PTSetting.ACTIONNAME[i];
                }
            }
        ...
        private void CopySelectedValues(PartPartProcessInfo selectedPpi, PartPartProcessInfo actualPpi, string machineId, string userno, string username)
        {
            // 後續工站使用 DB 重新取出的新 ppi；selectedPpi 只用來定位 processOrder，不帶舊箱狀態。
            int scanCount = Convert.ToInt32(actualPpi.ReadyCount);
            actualPpi.ScanCount = scanCount;
            actualPpi.BoxCount = scanCount;
            actualPpi.OriginalScanCount = scanCount;
            // 後續工站只是自動代刷，不再重複做分箱；分箱後的新 partno_id 已在進入本方法前決定。
            actualPpi.SplitQRCode = "";
            actualPpi.Eligibility = selectedPpi.Eligibility;
            actualPpi.Ineligibility = selectedPpi.Ineligibility;
            actualPpi.IneligibilityQRCode = selectedPpi.IneligibilityQRCode;
            actualPpi.ProcessList = selectedPpi.ProcessList;
            actualPpi.CustomerProcessList = selectedPpi.CustomerProcessList;
            actualPpi.MachineId = machineId;
            actualPpi.DesignateMachineId = machineId;
            actualPpi.UserId = userno;
            actualPpi.UserName = username;
        }

        private string ResolveFollowingPartNoId(PartPartProcessInfo ppi, QRCode qrcode)
        {
            if (!string.IsNullOrEmpty(ppi.SplitQRCode) && !string.Equals(ppi.SplitQRCode, ppi.QRCode, StringComparison.OrdinalIgnoreCase))
            {
                QRCodeInfo splitQrCodeInfo = qrcode.GetQRCodeByNo(ppi.SplitQRCode);
                if (splitQrCodeInfo != null && !string.IsNullOrEmpty(splitQrCodeInfo.PartNoId))
                {
                    return splitQrCodeInfo.PartNoId;
                }
            }

            return ppi.PartNo_Id;
        }

        private string SaveFollowingSelectedProcesses(List<PartPartProcessInfo> currentSelectedSource, List<PartPartProcessInfo> followingSelectedSource, string finalActionId, DateTime scanTime, double workhours, string userno, string username, string machineId, IBLLOutSourceDetail outsourceDetail)
        {
            if (followingSelectedSource == null || followingSelectedSource.Count == 0)
            {
                return Lang.SAVE_SUCCESS;
            }
            ...
            foreach (var processGroup in currentSelectedSource
                .GroupBy(m => new { m.ModuleId, m.PartNo_Id })
                .OrderBy(m => m.Key.ModuleId)
                .ThenBy(m => m.Key.PartNo_Id))
            {
                PartPartProcessInfo currentSelectedPpi = processGroup.OrderBy(m => m.ProcessOrder).First();
                string activePartNoId = ResolveFollowingPartNoId(currentSelectedPpi, qrcode);
                List<PartPartProcessInfo> followingGroup = followingSelectedSource
                    .Where(m => m.ModuleId.Equals(currentSelectedPpi.ModuleId) && m.PartNo_Id.Equals(currentSelectedPpi.PartNo_Id))
                    .OrderBy(m => m.ProcessOrder)
                    .ToList();

                foreach (PartPartProcessInfo selectedFollowingPpi in followingGroup)
                {
                    foreach (string actionId in BuildFollowingActionSequence(finalActionId))
                    {
                        // selectedFollowingPpi 來自畫面，分箱後它的 processno 仍是原箱舊記錄；
                        // 這裡必須用正在往下走的 activePartNoId + ProcessOrder 重新取 DB 新記錄。
                        PartPartProcessInfo actualPpi = partPartProcess.getPartProcessInfo(selectedFollowingPpi.ModuleId, activePartNoId, selectedFollowingPpi.ProcessOrder);
                        if (actualPpi == null)
                        {
                            return $"廠批[{selectedFollowingPpi.ModuleId}]圖號[{activePartNoId}]工序編號[{selectedFollowingPpi.ProcessOrder}]不存在";
                        }
                        if (!actualPpi.PartNo_Id.Equals(activePartNoId))
                        {
                            return $"廠批[{selectedFollowingPpi.ModuleId}]圖號[{activePartNoId}]工序編號[{selectedFollowingPpi.ProcessOrder}]未取得分箱後新工站";
                        }

                        ModuleWorkFlow.Model.ProcessInfo stepProcessInfo = new BLL.Process().GetProcessInfoById(actualPpi.ProcessId);
                        CopySelectedValues(selectedFollowingPpi, actualPpi, machineId, userno, username);
                        IList partHoldInfos = actionId.Equals("KAISHI") && actualPpi.StatusId.Equals("Holdon")
                            ? BuildSplitStartInfos(new List<PartPartProcessInfo> { actualPpi }, scanTime)
                            : new ArrayList();
                        string saveMsg = scanbar.updatePartProcessInfo(
                            new List<PartPartProcessInfo> { actualPpi },
                            actionId,
                            ...
                            false,
                            scanTime);
                        if (!saveMsg.Equals(Lang.SAVE_SUCCESS))
                        {
                            return saveMsg;
                        }
                    }

                    PartPartProcessInfo completedPpi = partPartProcess.getPartProcessInfo(selectedFollowingPpi.ModuleId, activePartNoId, selectedFollowingPpi.ProcessOrder);
                    if (completedPpi == null)
                    {
                        return $"廠批[{selectedFollowingPpi.ModuleId}]圖號[{activePartNoId}]工序編號[{selectedFollowingPpi.ProcessOrder}]不存在";
                    }
                    if (!completedPpi.PartNo_Id.Equals(activePartNoId))
                    {
                        return $"廠批[{selectedFollowingPpi.ModuleId}]圖號[{activePartNoId}]工序編號[{selectedFollowingPpi.ProcessOrder}]保存後未取得分箱後新工站";
                    }

                    CopySelectedValues(selectedFollowingPpi, completedPpi, machineId, userno, username);
                    ...
                    string boxMsg = boxlog.boxOperation(completedPpi, finalActionId, completedPpi.SplitQRCode, scanTime, new PartOutSourceDetail(), outsourceApplyDesigninfo);
                    if (!string.IsNullOrEmpty(boxMsg) && !boxMsg.Equals(Lang.SAVE_SUCCESS))
                    {
                        return boxMsg;
                    }

                    // 一站完整保存後再進下一站；後續站沿用當前正在往下走的箱號，不重複依 SplitQRCode 換箱。
                    activePartNoId = completedPpi.PartNo_Id;
                }
            }
        ...
        private string selectPart(string content, ModuleWorkFlow.Model.ProcessInfo processInfo)
        {
            ...
            try
            {
                ...
                {
                    errmsg = scanbar.selectModulePartWithSub(pmoduleid, appartno, hprocessno, (Hashtable)Session["hstatus"], processInfo, lab_hiddenActionId.Text, lab_machine_value.Text, Eligibility, Ineligibility, podi);
                }
            ...
            if (!lab_hiddenActionId.Text.Equals(""))
            {
                if (lab_hiddenActionId.Text.Equals("ZANTING") || lab_hiddenActionId.Text.Equals("JIESHU"))
                {
                    ...
                    foreach (System.Collections.DictionaryEntry objDE in hprocessno)
                    {
                        ...
                        if (nextppi != null)
                        {
                            ppi.NextProcess = Utility.Methods.TranslateProcessName(nextppi.ProcessId, new BLL.Process(), new ProcessCustomer());
                        }
                        source.Add((PartProcessInfo)objDE.Value);
                    }
        ...
        private void btn_submit_Click(object sender, System.EventArgs e)
        {
            ...
            //end of for 
            List<PartPartProcessInfo> currentSelectedSource = source;
            List<PartPartProcessInfo> followingSelectedSource = new List<PartPartProcessInfo>();
            if (actionid.Equals("JIESHU") || actionid.Equals("XIAODAN"))
            {
                string validateMsg = ValidateContinuousSelectedProcesses(source);
                if (!string.IsNullOrEmpty(validateMsg))
                {
                    Label_Message.Text = validateMsg;
                    return;
                }

                currentSelectedSource = GetCurrentSelectedProcesses(source);
                followingSelectedSource = GetFollowingSelectedProcesses(source);
            ...
            try
            {
                ...
                if (Label_Message.Text.Equals(Lang.SAVE_SUCCESS))
                {
                    ...
                    if ((Label_Message.Text.Equals(Lang.SAVE_SUCCESS) || string.IsNullOrEmpty(Label_Message.Text)) && (actionid.Equals("JIESHU") || actionid.Equals("XIAODAN")) && followingSelectedSource.Count > 0)
                    {
                        string followingSaveMsg = SaveFollowingSelectedProcesses(currentSelectedSource, followingSelectedSource, actionid, scanTime, Convert.ToDouble(lab_hiddenWorkHours.Text), userno, username, lab_machine_value.Text, new OutSourceDetail());
                        if (!followingSaveMsg.Equals(Lang.SAVE_SUCCESS))
                        {
                            Label_Message.Text = followingSaveMsg;
                            return;
                        }
                    }
